...
# added b/c color was not working on the docker logs and docker run -it
RED=''; GREEN=''; CYAN=''; NC=''
...
GO_BINARY="${GO_BINARY:-/usr/bin/erin-del-vid}"
...
health_check "Web server" "$PUBLIC_URL"

# === Start Go backend server ===
log "Starting Go backend on port $GO_PORT (CORS: $CORS_ORIGIN)"
"$GO_BINARY" -port="$GO_PORT" -cors-origin="$CORS_ORIGIN" -media="/srv/videos" &
GO_PID=$!
